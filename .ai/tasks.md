...
- [x] **Security audit & hardening (0.0.20)** — ran the `vibe-check/AI-CHECKLIST.md` audit (17 categories; reports + plans in `security/`). Fixed: **TMDB key removed from the browser bundle** (server-side `functions/api/tmdb.js` proxy injects key from `TMDB_API_KEY` secret, allow-listed path = SSRF-safe); **security headers** (CSP, HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy) via global middleware + `_headers` for static; **CORS wildcard → explicit allowlist**; **global error catch** (generic 500, details logged); **XSS fix** in `episodes.js` error branch (unescaped `err.message`) + poster-attr escaping across render modules; **password hashing hardened** to versioned PBKDF2 with 310k iterations + auto-rehash-on-login for legacy hashes. Audit verified PASS for DATABASE_ACCESS (D1 server-side scoping), AUTH_MIDDLEWARE, ACCESS_CONTROL, CSRF (SameSite=Lax), RATE_LIMITING, SQL_INJECTION, DEPENDENCIES (pnpm audit clean), N/A for payments/uploads.
